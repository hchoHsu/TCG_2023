...
void general ( std::string exec_name0, std::string exec_name1, int round, bool gui ) {
	...
	if ( exec_name0.size() == 0 ) {
		...
		player[0] = new AGENT(exec_name0, 2);
	...
	if ( exec_name1.size() == 0 ) {
		...
		player[1] = new AGENT(exec_name1, 2);
	}
	...
	while ( restart and round>0 ) {
		...
		player[0]->init(init_pos, (who_start==0)?'f':'s', b.dice_seq);
		player[1]->init(init_pos, (who_start==1)?'f':'s', b.dice_seq);
...
// start playing
	...
	// display
			...
			usleep(FPS*1000000);
			// change
			if ( b.winner() == Color::OTHER ) { CLR_LINES(BOARD_SZ-1+8); }
		}
