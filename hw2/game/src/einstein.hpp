...
#define NUM_DICE_SEQ 60
#define DICE_SEQ_LEN 21
...
// [0] = Player0, [1] = Player1
const int dx[2][4] = {{0, 1, 1, -1}, { 0, -1, -1,  1}};
const int dy[2][4] = {{1, 0, 1,  1}, {-1,  0, -1, -1}};
...
void Get_Dice_Seq(std::string &dice_seq) {
	FILE *fp;
	std::random_device rd;
	std::mt19937 gen(rd());
	std::uniform_int_distribution<> distrib(0, NUM_DICE_SEQ-1);
	int index = distrib(gen);
	char dice;

	if ((fp = fopen("dice_seq.txt", "r")) == nullptr) {
		perror("fopen: dice_seq.txt");
		exit(1); 
	}
	for (int i = 0; i < index * DICE_SEQ_LEN; i++) {
		fscanf(fp, " %*d");
	}
	for (int i = 0; i < DICE_SEQ_LEN; i++) {
		fscanf(fp, " %c", &dice);
		dice_seq += dice;
	}
	fclose(fp);
...
// generate initial cubes
		...
		Get_Dice_Seq(dice_seq);  // initialize the dice sequence
		// history.clear();
	}

	// called by the agent
	_board ( std::string top_left, std::string bottom_right, std::string dice_seq ) noexcept {
	...
	int state () const noexcept {
		...
		if ( now[B_CORNER].c != nullptr and now[B_CORNER].c->c == Color::R ) return 1;
		if ( now[R_CORNER].c != nullptr and now[R_CORNER].c->c == Color::B ) return 2;
	...
	VII move_gen(int num, PII &pos) const noexcept {
		Color color = turn();
	...
	// vectors of (num, dir)
	VII move_gen_all () const noexcept { // allow self eating
		...
		int cur_num = dice_seq[history.size() % DICE_SEQ_LEN] - '0';
		PII pos = find(color, cur_num);
		if (pos.first == -100) {
			for (int num = cur_num-1; num >= 0; num--) {
				pos = find(color, num);
				if (pos.first != -100) {
					VII tmp = this->move_gen(num, pos);
					res.insert(res.end(), tmp.begin(), tmp.end());
					break;
				}
			}
			for (int num = cur_num+1; num < NUM_CUBE; num++) {
				pos = find(color, num);
				if (pos.first != -100) {
					VII tmp = this->move_gen(num, pos);
					res.insert(res.end(), tmp.begin(), tmp.end());
			...
			VII tmp = this->move_gen(cur_num, pos);
			res.insert(res.end(), tmp.begin(), tmp.end());
		}
...
struct _board_gui : BOARD {
	...
	_board_gui ( std::string a, std::string b, std::string c ) : BOARD(a, b, c) {}
...
// change
	...
	int next = who;  // next player  // 0: red, 1: blue
	os << "\nnext dice:";
	for (int i = 0; i < DICE_SEQ_LEN; i++) {
		char dice = b_gui.dice_seq[(b_gui.history.size() + i) % DICE_SEQ_LEN];
		if (next == 0)  // red
			os << "\033[31m " << dice << "\033[m";
		else  // blue
			os << "\033[34m " << dice << "\033[m";
		next ^= 1;
	}
	os << '\n';
